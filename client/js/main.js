...
function proCardHTML(p) {
  ...
  const typeLabels = {
    ...
    physiotherapist: '🧘 PHYSIO', cleaner: '🧹 CLEANER', ac: '❄️ AC REPAIR',
    appliances: '📺 APPLIANCES', carpenter: '🔨 CARPENTER', painter: '🎨 PAINTER', pest: '🐜 PEST CONTROL'
  };
  const typeLabel = typeLabels[p.type] || 'PROFESSIONAL';
  const typeClass = p.type === 'electrician' ? 'elec' : p.type;

  const revs = (DB.reviews()[p.id] || []);
  const rating = revs.length ? (revs.reduce((s, r) => s + r.stars, 0) / revs.length).toFixed(1) : (p.rating || '5.0');
  const stars = '★'.repeat(Math.floor(+rating));

  const tags = (p.services || []).slice(0, 3).map(s =>
    `<span class="svc-tag svc-tag-${typeClass}">${s}</span>`
  ).join('') + (p.services.length > 3 ? `<span class="svc-tag svc-tag-${typeClass}">+${p.services.length - 3}</span>` : '');
  ...
  <div class="pro-card ${typeClass}" id="card-${p.id}">
    <div class="card-top">
      <span class="card-type-badge badge-${typeClass}">${typeLabel}</span>
      ${p.verified ? '<span class="badge-verified">✓ Verified</span>' : ''}
      <div class="card-avatar-row">
        <div class="card-avatar av-${typeClass}">${avatarContent}</div>
        <div>
          <div class="card-pname">${p.name}</div>
          <div class="card-shop">${p.shop || 'Individual Professional'}</div>
    ...
    <div class="card-body">
      ...
      <div class="card-metrics">
        ...
        <div class="metric-item">
          <div class="metric-val">${p.reachedHomeCount || 0}</div>
          <div class="metric-lab">REACHED</div>
        </div>
      </div>

      <div class="card-fee">
        <span>Visit fee: <span class="card-fee-val">₹${p.fee}</span></span>
      ...
      <div class="card-btns">
        ...
        <button class="cbtn cbtn-solid-${typeClass}" onclick="openBooking('${p.id}')">Book Now</button>
      </div>
    </div>
  </div>`;
}
...
function confirmBooking(pid) {
  ...
  toast(`Request sent to ${p.name}! Waiting for acceptance...`, 'info');
  showTracking(p, bk);
}
...
function showTracking(p, bk) {
  ...
  if (window.trackingPoll) clearInterval(window.trackingPoll);

  const renderModal = (currentBk) => {
    const isPending = currentBk.status === 'pending';

    document.getElementById('booking-modal').innerHTML = `
      <div class="modal-hdr">
        <div>
          <div class="modal-ttl">${isPending ? '⏳ Waiting for Confirmation' : (isElec ? '⚡ Professional En Route!' : '🔧 Professional En Route!')}</div>
          <div class="modal-sub">${p.name} · ${currentBk.svc}</div>
        </div>
        <button class="modal-x" onclick="closeOverlay('booking-overlay'); if(window.trackingPoll) clearInterval(window.trackingPoll);">✕</button>
      </div>
      <div class="modal-body" style="padding-top:20px">
        ${isPending ? `
          <div style="text-align:center; padding:40px 20px;">
            <div class="spinner" style="width:40px; height:40px; border:3px solid rgba(234, 179, 8, 0.2); border-top-color:var(--amber); border-radius:50%; animation: spin 1s linear infinite; margin:0 auto 20px;"></div>
            <div style="font-weight:700; font-size:18px; color:var(--white); margin-bottom:8px;">Request Sent to ${p.name}</div>
            <div style="color:var(--ash); font-size:14px;">We have notified ${p.name}. They will accept your request within 2 minutes.</div>
            <div style="margin-top:30px; background:rgba(255,255,255,0.03); padding:15px; border-radius:12px; border:1px solid var(--border-subtle);">
              <div style="font-size:12px; color:var(--ash); text-transform:uppercase; letter-spacing:1px; margin-bottom:5px;">Professional Info</div>
              <div style="font-weight:700; font-size:16px; color:var(--amber);">${p.name}</div>
              <div style="font-size:13px;">${p.rating} ⭐ · ${p.exp} yrs Experience</div>
            </div>
          </div>
        ` : `
          <!-- STATUS STEPS -->
          <div class="bk-steps">
            <div class="bk-step"><div class="bk-dot done">✓</div><span class="bk-step-lbl">Requested</span></div>
          ...
          <div class="eta-box">
            ...
            <div>
              <div class="eta-time" id="eta-val">${currentBk.eta} min</div>
              <div class="eta-lbl">${p.name} has accepted!</div>
              <div class="eta-sub">${currentBk.dist} km away · ${p.area}, ${p.city}</div>
            </div>
          </div>

          <!-- MAP (Simplified for space) -->
          <div class="map-box" style="height:150px">
            <div class="map-bg"></div>
            <div class="map-dest" style="left:70%;top:30%">📍</div>
            <div class="map-worker" id="map-worker" style="left:20%;top:60%">${isElec ? '⚡' : '🛵'}</div>
          </div>

          <!-- PROFESSIONAL INFO -->
          <div style="background:rgba(255,255,255,0.05); border-radius:12px; padding:14px 16px; display:flex; align-items:center; justify-content:space-between; gap:12px;">
            <div>
              <div style="font-weight:700; font-size:14px">${p.name} ${emoji}</div>
              <div style="font-size:13px; color:var(--ash)">Verified Expert · 📞 ${p.phone}</div>
            </div>
            <a href="tel:${p.phone}" style="background:var(--amber); color:var(--void); padding:10px 18px; border-radius:10px; font-size:13px; font-weight:700; text-decoration:none;">📞 Call</a>
          </div>
        `}
      </div>
    `;
    // Animate worker if confirmed
    if (!isPending) {
      let step = 0;
      const totalSteps = 100;
      const tx = 70, ty = 30; // Destination
      let wx = 20, wy = 60; // Start

      const animInterval = setInterval(() => {
        step++;
        const frac = step / totalSteps;
        const curWx = wx + (tx - wx) * frac;
        const curWy = wy + (ty - wy) * frac;

        const wEl = document.getElementById('map-worker');
        const et = document.getElementById('eta-val');
        if (wEl) { wEl.style.left = curWx + '%'; wEl.style.top = curWy + '%'; }
        if (et) {
          const remMin = Math.max(1, Math.round(bk.eta * (1 - frac)));
          et.textContent = remMin + ' min';
        }

        if (frac >= 1) {
          clearInterval(animInterval);
          toast('Professional has arrived!', 'ok');
        }
      }, 2000);
    }
  };
  ...
  window.trackingPoll = setInterval(() => {
    const latestBks = DB.bookings();
    const currentBk = latestBks.find(b => b.id === bk.id);
    if (currentBk && currentBk.status === 'confirmed') {
      clearInterval(window.trackingPoll);
      toast('Professional has accepted your booking!', 'ok');
      renderModal(currentBk);
    }
  }, 3000);
}
...
function registerPro(e) {
  ...
  try {
    ...
    setTimeout(() => {
      closeRegModal();
      toast('Registration Successful! Your profile is live.', 'ok');
